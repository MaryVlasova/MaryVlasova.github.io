...
window.onload = function () {
	...
	gotos.forEach((link, i, links) => {		
		...
		link.addEventListener('click', function(e) {
			...
			const offsetPosition = elementPosition - topOffset*2;
	
			window.scrollBy({
				top: offsetPosition,
				behavior: 'smooth'
			});
			// add active class for mobiles and small screens
			if(isMobile.any() || !(getComputedStyle(menuIcon).display == "none")) {
				disactiveMenuItmes(links);
...
const clientsObserver = new IntersectionObserver(entries => {
	entries.forEach(entry => {	
		if (entry.isIntersecting) {
			const clinents = entry.target.querySelectorAll('.client');
			let delay = 0;
			clinents.forEach(function(clinent) {
				setTimeout(function () {
					clinent.classList.add('client-animation');
				}, delay);	
				delay += 100;
			});	
...
clientsObserver.observe(document.querySelector('#clients'));

if (!isMobile.any()) { 
	
	const gotoObserver = new IntersectionObserver(entries => {	 
		...
		entries.forEach(entry => {	
			...
			const links = document.querySelectorAll("._goto-block[href='#"+entry.target.id+"']");	
	...
	gotos.forEach(function (goto)  {
		const href =  goto.getAttribute('href').replace('#', '');
		if (href) {	
			gotoObserver.observe(document.querySelector('#' + href));
		}		
	})

} 

function disactiveMenuItmes(allMenuItems) {
